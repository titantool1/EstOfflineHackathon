...
이 문서는 `b2e7878` 시점의 기록이다. 실제 상담 종료 저장까지 연결한 후속 결과는 [조건 저장 연결 통합검증](condition-save-integration.md)에 있다.
...
이 검사 당시 조건 저장 1단위는 아직 production 저장 어댑터·Spring HTTP API·상담 종료 화면에 연결되지 않았다. 이번 통합검증은 실제 상담 조건 저장 완료를 뜻하지 않는다. 다음은 인증된 owner로 조건을 FactChange에 매핑하고 충돌/중복 요청 결과를 처리하는 Spring 저장 경계를 연결하는 한 단위다. 대화별 메모리를 유지한다.
