...
상담 시작 때 인증된 본인의 지원 조건 정의와 DB 초기값을 읽는다. 매 턴 모델의 구조화 결과를 코드가 검증하고 기존 유효값과 비교한 뒤 상담 메모리에 반영한다. 모호한 대상은 쓰지 않고 되묻되 독립적으로 확실한 정보는 함께 반영한다. 이 단계는 선택적인 조건 갱신 도구 호출에 의존하지 않는다. [제품 연결·실제 QA 결과](qa-condition-memory-2026-09-18.md).
...
같은 탭의 새로고침이나 화면 재진입은 상담 종료로 취급하지 않는다. 탭에 남긴 무작위 식별자로 인증된 본인의 진행 상담과 표시용 대화를 다시 조회한다. 응답 생성 중 화면 연결이 끊겨도 기존85초 생성 제한 안에서 작업을 마치며 복원 화면은 같은 작업을 조회한다. 조회 자체로90초 무활동 마감 시각을 연장하지 않는다. 새상담·실제 만료 뒤에는 기존 대화를 다시 열지 않는다. 브라우저에는 식별자만 저장하고, 전체 대화는 DB에 영구 저장하지 않는다.
