...
`work/private-facts`는 기존 회원 사실 8개 테이블을 유지하면서 개인값을 Spring에서 AES-256-GCM으로 암호화한다. 사용자·가구·구성원·주택·차량 UUID와 구조 관계, 행 수는 DB에 남는다. 계정 이메일·닉네임·비밀번호 해시는 이번 변경 범위 밖이다. 팀 운영은 V12까지 이관·배포했다. [배포·키·백업 기록](deployment-2026-09-18.md). 새 환경 적용 전에는 아래 키 준비와 유지보수 이관이 필요하다.
...
- revision `0`은 신규 행 생성(빈 profile anchor에도 사용), 양수는 정확한 기존 revision 확인, `null`은 기존 행의 최신 값에 부분 병합이다. 일반 동네 저장만 현재 UI에 맞춰 `null`을 사용한다. 상담 저장은 조회 당시 baseline과 현재 값을 비교하며 [종료 정책](condition-save.md)을 따른다.
...
`update_conditions`는 계속 대화 메모리만 변경한다. 대화 종료 시점의 명시적 변경은 별도 application 서비스가 이 포트와 V12 암호화 receipt를 같은 트랜잭션으로 저장한다. 종료 정의·모름/거절·동일 사실 충돌은 [상담 종료 정책](condition-save.md)을 따른다. 새 대화의 기존 `ConditionContext` JSON 계약은 유지한다. 공개 조건 매핑을 먼저 읽고 필요한 소스/선택 대상만 복호화하며 조회 전체는 `ConditionContextService.load`가 여는 REPEATABLE_READ snapshot이다.
...
1. 적용할 릴리스의 migration 목록을 확인한다. 현재 릴리스는 기존 V1~V7(미션·관심사 V4/V6/V7 포함)에 V8/V9/V10 암호화, V11 미션 모드, V12 상담 저장 receipt를 포함한다. V7 기존 회원·관심사·추천·활동 기록의 V10 이관 보존을 격리 PG에서 검증했다. 낮은 번호의 누락 migration이 있는 DB는 기본 기동으로 넘기지 않는다. 팀 운영의 V5 이후 V4 누락은 복원 리허설을 거친 유지보수 기동에서만 out-of-order를 허용하고 정상 기동에서 제거했다.
...
8개 사실 테이블뿐 아니라 V12 condition_save_receipts의 암호문도 같은 키링을 사용한다. 사실 행의 빈 patch 재암호화는 receipt를 갱신하지 않으므로 이것만으로 구 키를 제거하면 안 된다. 모든 암호화 저장소의 key_id별 행 수·복호화 대조와 구 키가 필요한 백업의 복구 정책까지 확인한 뒤에만 구 키를 제거한다. 키 소실은 데이터 복구 불가로 이어진다. GCM은 소유자·대상·버전·revision의 독립적인 변조나 서로 다른 문맥의 암호문 혼합을 검출한다. 다만 과거에 유효했던 한 행의 envelope 전체(key_id/nonce/ciphertext/revision 포함)를 되돌리는 재생이나 전체 DB snapshot 복원을 감지하는 별도 원장은 아니다.
