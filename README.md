...
우리 사전 실험의 Next.js·Spring·PostgreSQL·ES/Nori·LangGraph·BGE-M3 구성을 팀 개발의 기준으로 준비한다. 현재 범위는 **실행 환경·기능별 모듈·화면 API 진입점 뼈대**까지다. 회원가입·로그인·닉네임과 사용자 조건 조회 코드를 추가했다. 실제 검색 도구와 기존 챗 화면 연결은 후속 단계다.
...
[기존 세션 방식의 회원 관리](docs/member-accounts.md)를 팀 구조에 연결했다. `/signup`에서 닉네임을 선택 입력하고 비우면 `에코쭙`+랜덤6자리로 저장한다. `/profile`에서 확인한다. 추가 Flyway V3는 다음 Spring 기동 때 적용된다. 코드·단위검사 완료이며 실제 DB/브라우저 통합검사와 실행 환경 배포는 아직이다.
