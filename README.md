...
우리 사전 실험의 Next.js·Spring·PostgreSQL·ES/Nori·LangGraph·BGE-M3 구성을 팀 개발의 기준으로 사용한다. 회원·개인화·미션과 LangGraph 대화 실행부를 연결했다. 아래 현재 통합검증 상태와 날짜가 지난 실행 기록을 구분해서 확인한다.

## 통합검증 상태 (2026-09-18)

`work/condition-save-connected`에 암호화·조건 저장 기반 `b2e7878`과 main `b0bfbe8`의 스트리밍·반응형·미션 두 영역(V11)을 통합했다. 새 상담 또는 응답 완료 후 90초 무활동에서 조건 변경을 Spring에 저장하고 대화를 초기화한다. 저장 실패·충돌·결과 미확인에도 초기화하며, 실제 저장 성공에만 “저장되었습니다”를 표시한다. [기능 정책](docs/condition-save.md).

Spring 96개(실제 PG 검사 3개 포함), 프런트 174개, 린트·프로덕션 빌드·타입 검사, 실제 API 10개·대화 연결 6개·브라우저 9개가 통과했다. V11과 V12 이관도 함께 확인했다. [결과·확인 한계·배포 전 단계](docs/condition-save-integration.md). main 반영·push·운영 배포는 아직 수행하지 않았다. 아래 2026-09-17의 구현·실행 표는 당시 기록이다.
...
이 브랜치의 Spring은 **암호화 키 파일이 필수**다. 최초 실행/기존 DB 갱신 전 [사용자 사실 보호 저장](docs/private-facts-storage.md)의 키 준비·V8~V10 유지보수 이관 절차를 완료한다. 기본 실행은 이관을 자동 승인하지 않는다.
