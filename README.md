...
미션·관심사의 main `6ab7fe9`와 암호화 `8734a38`를 포함한 조건 저장 `4622cf7`를 `work/member-facts-integration`에 통합했다. Spring 85개(실제 PG 이관·기동 3개 포함), 프론트 143개, 타입·린트·빌드와 실제 Next→Spring→DB 기본 동작 검사를 통과했다. [통합검증 범위와 후속](docs/member-facts-integration.md).

조건 저장은 대화별 변경 수집과 저장 포트 단계까지다. 실제 상담 종료→Spring 저장 API 연결은 다음 구현 단위다. 이번 검증은 격리 시험 환경에서 수행했으며 운영 반영 상태를 변경하지 않았다. 아래 2026-09-17의 구현·실행 표는 당시 기록이다.
