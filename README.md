...
우리 사전 실험의 Next.js·Spring·PostgreSQL·ES/Nori·LangGraph·BGE-M3 구성을 팀 개발의 기준으로 준비한다. 현재 범위는 **4단계: 실행 환경·기본 연결과 기능별 모듈 뼈대**까지다. 실제 검색 도구·인증·업무 API와 기존 챗 화면 연결은 후속 단계다.
...
3. [기능별 모듈 뼈대와 책임 경계](docs/architecture.md) 준비. DB 설계·조회/저장 계약은 별도 작업 결과와 맞춘다.
4. 각 단계의 검증이 끝나면 main에 반영하고, 이후 단계에서 전체 기동·키 전달·기존 화면 보존을 확인.

기본 추천은 코드, 자연어 해석·질문·답변은 AI 흐름에 둔다. 현재 상태 검사는 외부 모델 API를 호출하지 않는다. 카탈로그 적재·실제 검색 도구·개별 기능 구현은 후속이다. `package-info.java`와 features 안내는 구현 위치 표시이며 기능 완료가 아니다. Flyway는 아직 연결하지 않았다.
