...
제도·행동·조건·출처 조회와 사용자 사실 저장을 검증한 SQL 묶음이다. PostgreSQL18.6에서 카탈로그129개, 기본 사용자55개, 상세56개를 검사한다. 이 검증본 전체를 팀 개발 환경의 Flyway V1/V2로 연결했다. [개발 DB 실행·접속·확인](development.md). 아래 검증 기록과 시험 한계는 당시 범위를 유지한다.
...
1. 전체 스키마·공개 자료·입력 매핑은 [Flyway 위치](../backend/src/main/resources/db/migration/README.md)의 V1/V2에서 설치한다. `./scripts/setup-db.sh`로 실제 팀 개발 DB를 준비한다.
2. `users(id uuid)` 참조와 지역 계층은 생성하되 사용자·시험 지역은 자동 적재하지 않는다. 실제 인증·공식 지역 연결은 후속 마이그레이션으로 맞춘다.
3. `profile`·`catalog` 모듈에서 저장·조회 포트와 인증된 사용자 소유권 검사를 연결한다. 기존 SQL의 사용자 ID 분리 검사가 앱 API 권한 검사를 대신하지 않는다.
4. 모델의 구조화된 답변을 조건 메모리·필요한 저장 경로와 연결한다.
