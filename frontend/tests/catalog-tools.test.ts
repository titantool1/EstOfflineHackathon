...
const page = { query: "컵", match_mode: "hybrid_rrf", offset: 0, limit: 10, has_more: false, items: [] };
...
function tools(data: unknown, status = 200, code = "DATABASE_UNAVAILABLE", observe?: (url: URL, init?: RequestInit) => void) {
  return createCatalogTools(createSpringClient({ baseUrl: "http://backend.test", fetch: async (input, init) => {
    ...
    if (url.pathname === "/api/auth/csrf") return Response.json({
      data: { token: "csrf-token", headerName: "X-CSRF-TOKEN" }, error: null, requestId: id,
...
test("DB and connection errors never become empty search", async () => {
  ...
  const down = createCatalogTools(createSpringClient({ baseUrl: "http://backend.test", fetch: async () => { throw new Error("down"); } }), async () => vector);
...
test("query and pinned-size vector are sent to the hybrid endpoint", async () => {
  const q = "컵 %_'";
  const result = await tools({ ...page, query: q }, 200, "", (url, init) => {
    assert.equal(url.pathname, "/api/catalog/actions/search");
    assert.equal(init?.method, "POST");
    assert.deepEqual(JSON.parse(String(init?.body)), { query: q, embedding: vector, limit: 10, offset: 0 });
    assert.equal(new Headers(init?.headers).get("X-CSRF-TOKEN"), "csrf-token");
    assert.equal(new Headers(init?.headers).get("Cookie"), "ECOTEAMSESSION=test-session");
...
test("embedding is required only for search and failures never become empty results", async () => {
  let calls = 0;
  const spring = createSpringClient({ baseUrl: "http://backend.test", fetch: async (_input, init) => {
    calls++;
    const id = new Headers(init?.headers).get("X-Request-Id")!;
    return Response.json({ data: detail, error: null, requestId: id }, { headers: { "X-Request-Id": id } });
  } });
  const api = createCatalogTools(spring);
  assert.equal((await api.execute("get_catalog_action", { programKey: "P", actionId: "A" })).status, "ok");
  await assert.rejects(api.execute("search_catalog", search), { code: "EMBEDDING_NOT_CONFIGURED" });
  const down = createCatalogTools(spring, async () => { throw new Error("embedding failed"); });
