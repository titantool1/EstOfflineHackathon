...
export function MissionProgressPanel({ revision }: { revision: number }) {
  ...
  return <section aria-label="내 미션 레벨" className="mb-7 rounded-3xl border border-[#d5e8cc] bg-white p-5 sm:p-6">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <div><h2 className="text-sm font-bold text-[#61745f]">나의 실천 레벨</h2>
          {current && <p className="text-2xl font-bold text-[#284527]">Lv.{current.level}</p>}
        </div>
      </div>
      {progress && <p className="text-sm text-[#61745f]">서로 다른 미션 <strong className="text-[#347b3d]">{progress.completedMissionCount}개</strong> 실천</p>}
    </div>
    {current && <>
      <div className="mt-4 flex flex-wrap justify-between gap-2 text-sm text-[#61745f]">
        <p>다음 레벨까지 <strong className="text-[#347b3d]">{current.remaining}개</strong></p>
      ...
      <div role="progressbar" aria-label="다음 레벨 진행도" aria-valuemin={0} aria-valuemax={current.required}
        ...
        className="mt-2 h-3 overflow-hidden rounded-full bg-[#edf3e8]">
        <div className="h-full rounded-full bg-[#61a751] transition-[width] duration-500 motion-reduce:transition-none"
          style={{ width: `${current.completedInLevel / current.required * 100}%` }} />
      </div>
      <p className="mt-3 text-xs leading-5 text-[#71816f]">새로운 미션을 실천할 때마다 성장해요. 같은 미션은 한 번만 반영돼요.</p>
    </>}
    {!progress && !error && <p role="status" className="mt-3 text-sm text-[#61745f]">실천 기록을 불러오는 중이에요.</p>}
    ...
    <div role="status" aria-live="polite" aria-atomic="true">
      {notice && <p className="mt-4 rounded-2xl bg-[#e9f6e4] px-4 py-3 text-sm font-bold text-[#347b3d]">
